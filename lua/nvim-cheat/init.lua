...
local function openCheatDetectType(self, language, query, disable_comment)
    if query == '' then
        return
    end
...
local function setupResultWindow(self, init_text, disable_comment)
    local function initCheatWindow(line, contextWiseWindowFunction)
        ...
        if contains(M.detectFileTypes, language) then
            openCheatDetectType(self, language, line, disable_comment)
        elseif not openCheat(self, line, disable_comment) then
